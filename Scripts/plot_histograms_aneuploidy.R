...
het_hist <- ggplot(sctab, aes(heterozygosity))+ 
  geom_histogram(aes(fill=aneuploidy_binary),
                 color="black", 
                 bins=50,
                 alpha=0.7)+
  scale_x_continuous(breaks=seq(0,0.01,0.001), limits=c(0,max(sctab$heterozygosity))) +
  ...
  scale_fill_discrete(name="", labels = c("Euploids", "Aneuploids"))+
  geom_vline(xintercept = 0.003,linetype="dotted")
het_hist
