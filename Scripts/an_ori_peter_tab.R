...
petertab$amp_response <- ifelse(petertab$Aneuploidy_type =="Deletion","No","Yes")
...
ploidymatrix <- as.matrix(table(petertab$Ploidy,petertab$aneuploidy_response), nrow=5, header = TRUE)
...
png("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/Ploidy/ploidy_propplot_peter.png", width = 6, height = 6, units = 'in', res = 300, bg='transparent')
p_plot
dev.off()
...
png("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/Heterozygosity/het_amp_peter.png", width = 6, height = 6, units = 'in', res = 300, bg='transparent')
het_hist
dev.off()
...
het_hist <- het_hist + geom_histogram(aes(fill=aneuploidy_response),
                                      color="black", 
                                      bins=30,
                                      alpha=0.7)+
  scale_x_continuous(breaks=seq(0,0.01,0.001), limits=c(0,0.01)) +
  ...
  theme(axis.title.y = element_text(face="bold", size=14))+
  theme(legend.position = "bottom")
png("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/Heterozygosity/2n_het_propplot_peter.png", width = 6, height = 6, units = 'in', res = 300, bg='transparent')
het_hist
dev.off()
...
oldecomatrix <- as.matrix(table(dip_oldecopeter$`Ecological origins`, dip_oldecopeter$aneuploidy_response),
                       nrow=length(unique(dip_oldecopeter$`Ecological origins`)), 
                       header = TRUE)
scmatrix <- oldecomatrix
scmatrix
fisher.test(scmatrix, simulate.p.value = TRUE)
...
orplot <- orplot + 
  ...
  ggtitle("Proportion of aneuploids per environment (>10 strains) in diploids")+
  theme(plot.title = element_text(face="bold",h=0.5))
orplot <- orplot +
  theme(
    panel.background = element_rect(fill = "transparent",colour = NA), 
...
png("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/Ecology/2n_old_eco_pplot_peter.png", width = 6, height = 6, units = 'in', res = 300, bg='transparent')
orplot
dev.off()
#########################################################################################################################################################
...
newecomatrix <- as.matrix(table(dip_newecopeter$new_eco, dip_newecopeter$aneuploidy_response), 
                          nrow=length(unique(dip_newecopeter$new_eco)), 
                          header = TRUE)
# convert df to matrix
scmatrix <- newecomatrix
...
orplot <- orplot + 
  ...
  theme(axis.text.y= element_text(color=my_colors))+
  ggtitle("Proportion of aneuploids per environment (>10 2n strains)")+
...
orplot <- orplot + theme(legend.position = "bottom")
png("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/Ecology/2n_new_eco_pplot_peter.png", width = 6, height = 6, units = 'in', res = 300, bg='transparent')
orplot
dev.off()
...
popmatrix <- as.matrix(table(poppeter$Clades, poppeter$aneuploidy_response),
...
dip_popmatrix <- as.matrix(table(dip_poppeter$Clades, dip_poppeter$aneuploidy_response),
                       nrow=length(unique(dip_poppeter$Clades)), 
                       header = TRUE)


# convert df to matrix
scmatrix <- dip_popmatrix
...
orplot <- orplot + 
  ...
  theme(axis.text= element_text(face="bold",color="gray50", size=10))+
  theme(axis.text.y= element_text(color=my_colors))+
  ggtitle("Proportion of aneuploids per clade (>10 2n strains)")+
...
orplot <- orplot + theme(legend.position = "bottom")
png("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/Clade/2n_clade_propplot_peter.png", width = 6, height = 6, units = 'in', res = 300, bg='transparent')
orplot
dev.off()
