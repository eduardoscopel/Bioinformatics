...
a<-sample(col_vector, 23)
a <- data.frame(color = a, lineage = unique(myY$lineage))
count <- 0
for(clade in unique(myY$lineage)){
  count <- count + 1
  for(i in 1:nrow(myY)){
    if(myY$lineage[i] == clade){
      myY$color[i] <- as.character(a$color[count])
    }
  }
}
...
col_list<-c(first_af1,
            ...
            twent9_ch,
            thirth_NA)
#col_list<-c("#9dec5f","#7570B3","#CAB2D6","#FFF2AE","#E5C494","#6A3D9A","#1F78B4","#BF5B17","#FDC086","#E7298A",
#            "#BC80BD","#33A02C","#E78AC3","#F2F2F2","#A6761D","#D95F02","#B2DF8A","#984EA3","red","#8DA0CB",
#            "#B3E2CD","#FB9A99","gray","orange","forestgreen","steelblue2","darkcyan","goldenrod3","#310041",
#            "#ffa804","#ff2b5e")

Qlist <- list.files("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/AlignedMatrices/1645028616/aligned.files/ready/")
popIndices <- as.numeric(tapply(seq_along(sort(myY$lineage)),sort(myY$lineage), max))+0.5
popMean <- as.numeric(tapply(seq_along(sort(myY$lineage)),sort(myY$lineage), mean))+0.5

plotList <- c()
counter <- 1
for(i in Qlist[2:40]){
  temp <- read.table(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/gainOnly/newRunQC40Miss10MAF1.0noRecomb/admixture/s12345/",i,sep = ""))
  temp <- temp[!(row.names(temp) == 602),]
  k <- length(temp)
  ...
  if(k %% 10 == 0){
    plotList[[counter]] <- ggplot(melteddf) + 
      ...
      scale_x_discrete(expand = c(0,0), breaks = temp$basename[popMean], labels = temp$lineage[popMean])+
...
plot_grid(plotlist = plotList[20:29], nrow = 10, rel_heights = c(1,1,1,1,1,1,1,1,1,2))
ggsave(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/plots/","s1_2-5",".png",sep = ""), 
       width=15, 
       height=15, 
       units="in",
       limitsize=FALSE, 
       dpi = 600)
plot_grid(plotlist = plotList[11:20], nrow = 10, rel_heights = c(1,1,1,1,1,1,1,1,1,2))
ggsave(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/plots/","s1_6-9",".png",sep = ""), 
       width=15, 
       height=15, 
       units="in",
       limitsize=FALSE, 
       dpi = 600)
plot_grid(plotlist = plotList[21:30], nrow = 10, rel_heights = c(1,1,1,1,1,1,1,1,1,2))
ggsave(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/plots/","s1_10-13",".png",sep = ""), 
       width=15, 
       height=15, 
       units="in",
       limitsize=FALSE, 
...
ggsave(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/plots/","s1_14-17",".png",sep = ""), 
       width=15, 
       height=15, 
       units="in",
       limitsize=FALSE, 
...
ggsave(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/plots/","s1_18-21",".png",sep = ""), 
       width=15, 
       height=15, 
       units="in",
       limitsize=FALSE, 
       dpi = 600)
plot_grid(plotlist = plotList[21:24], nrow = 4, rel_heights = c(1,1,1,1.5))
ggsave(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/plots/","s1_22-25",".png",sep = ""), 
       width=15, 
       height=15, 
       units="in",
       limitsize=FALSE, 
       dpi = 600)
plot_grid(plotlist = plotList[25:28], nrow = 4, rel_heights = c(1,1,1,1.5))
ggsave(paste("/Users/es47540/Documents/Papers_Scopel/GWAS/diploids/admixture/t500bp/s1/plots/","s1_26-29",".png",sep = ""), 
       width=15, 
       height=15, 
       units="in",
       limitsize=FALSE, 
       dpi = 600)
plot_grid(plotlist = plotList[29], nrow = 1)
