...
ploidymatrix <- ploidymatrix[,c(4,5,2,3,1)]
ploidymatrix
...
for(i in 1:nrow(ploidymatrix)){
  if(i==1){
    ...
    proportion_1only <- c((ploidymatrix[i,5])/(total_ploidy[i]))
  }
  else{
    ...
    proportion_1only <- c(proportion_1only,(ploidymatrix[i,5])/(total_ploidy[i]))
  }
}
...
ploidymatrix <- cbind(ploidymatrix, proportion_1only)
...
for(i in 1:nrow(ploidymatrix)){
  odds_ratio[1,i] <- as.numeric(fisher.test(
    matrix(c(ploidymatrix[i,1]+ploidymatrix[i,3],
             ploidymatrix[i,2]+ploidymatrix[i,4] + ploidymatrix[i,5],
             sum(ploidymatrix[,1])+sum(ploidymatrix[,3])-ploidymatrix[i,1]+ploidymatrix[i,3],
             sum(ploidymatrix[,2])+sum(ploidymatrix[,4])+sum(ploidymatrix[,5])-ploidymatrix[i,2]+ploidymatrix[i,4] + ploidymatrix[i,5]),
           nrow=2,
  ...
  odds_ratio[2,i] <- as.numeric(fisher.test(
    matrix(c(ploidymatrix[i,2]+ploidymatrix[i,3],
             ploidymatrix[i,1]+ploidymatrix[i,4] + ploidymatrix[i,5],
             sum(ploidymatrix[,2]+ploidymatrix[,3])-ploidymatrix[i,2]+ploidymatrix[i,3],
             sum(ploidymatrix[,1])+sum(ploidymatrix[,4])+sum(ploidymatrix[,5])-ploidymatrix[i,1]+ploidymatrix[i,4] + ploidymatrix[i,5]),
           nrow=2,
...
for(row in 1:nrow(ploidymatrix)){
  if(pcat[row]==1){
    pcat[row]<- "Haploids"
    pLabels <- append(pLabels,paste(pcat[row]," (",ploidymatrix[row,6],")",sep=""))
  }
  if(pcat[row]==2){
    pcat[row]<- "Diploids"
    pLabels <- append(pLabels,paste(pcat[row]," (",ploidymatrix[row,6],")",sep=""))
...
ploidydf <- data.frame(xAxis = seq(1,5,1),
                       ...
                       prop = c(ploidymatrix[,7],-ploidymatrix[,8],ploidymatrix[,9],-ploidymatrix[,9]),
                       odds = c(odds_ratio[1,],odds_ratio[2,],odds_ratio[1,],odds_ratio[2,]),
                       p = c(p_values[1,],p_values[2,],p_values[1,],p_values[2,]))
ploidydf$fdr[c(9, 19,  4, 14,  7, 17, 10, 20,  3, 13,  6, 16,  5, 15,  8, 18,  1, 11,  2, 12)] <- mt.rawp2adjp(ploidydf$p, proc = "BH", alpha = 0.05)$adjp[,2]
...
fisherplot<- ggplot(ploidydf, aes(x= xAxis, y = prop)) + 
  geom_rect(aes(ymin= -sum(ploidymatrix[,2])/sum(ploidymatrix[,6]), ymax=sum(ploidymatrix[,1])/sum(ploidymatrix[,6]),xmin=-Inf,xmax=Inf),
            color="#e0e1e2", 
            alpha=0.03)+
...
png("Documents/GitHub/eduardo/aneuploidy/plots/peter/Ploidy/Fig1C_ploidy.png",width = 3, height = 5, units = "in",bg = "white", res=300)
fisherplot
dev.off()
