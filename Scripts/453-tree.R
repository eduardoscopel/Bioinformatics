...
petertab12 <- petertab7[petertab7$basename %in% tokeep2$V1]
...
ptree <- ggtree(y,size=0.2,layout="rectangular") + 
  geom_tree(show.legend = FALSE, size=0.5)+
...
ptree <- ptree + theme(legend.position = c(0.4,0.2), 
...
ptree <- ptree + geom_hilight(node = 830, fill = "black", alpha=0.2, extend = 0.0074)+ # French dairy + African Beer
  ...
  geom_hilight(node = 460, fill = "black", alpha=0.1, extend = 0.0079)+ # Mixed origin
  geom_hilight(node = 748, fill = "black", alpha=0.1, extend = 0.01515) + # sake
  geom_hilight(node = 463, fill = "black", alpha=0.2, extend = 0.0121) # Mosaic beer

ptree <- ptree + scale_x_continuous(limits = c(0, 0.055))+scale_y_continuous(expand = c(0.01,0.01))
ggsave("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/trees/FigS3.v3.png", width=9, height = 12, units="in",limitsize = FALSE, dpi = 600)

ggsave("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/trees/FigS3.v3.eps",device = cairo_ps, width=9, height = 12, units="in",limitsize = FALSE, dpi = 600)
