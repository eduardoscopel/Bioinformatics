...
ptree <- ggtree(y,size=0.2,layout="rectangular") + 
  ...
  scale_color_manual(values=c("#de77ae","#4d9221","#1f78b4"))+
  new_scale_color()+
  geom_tiplab(size=2,aes(color=clade),align = TRUE,linesize = 0, show.legend = FALSE, x=0.042) +
  scale_colour_manual(values = cladecolors)+
  geom_point2(aes(shape=amplification),size=1, color = 'red',na.rm = TRUE, x = 0.041)+
  scale_shape_manual(values=16)
...
ptree <- ptree + theme(legend.position = c(0.3,0.9), 
                       legend.key.height = unit(0.1, "lines"),
                       legend.key.width = unit(1, "lines"),
                       legend.text = element_text(size=7), 
                       legend.title = element_text(size=7),
...
saketree <- viewClade(ptree, 1066)+geom_treescale(x=0.038,y=135, width=0.005, offset = 0.2, linesize = 0.2, fontsize=3)
saketree + theme_classic()

ggsave("/Users/es47540/Documents/GitHub/eduardo/aneuploidy/plots/peter/trees/629_sake_chrhm.png", width=5, height = 5, units="in",limitsize = FALSE, dpi = 600)
